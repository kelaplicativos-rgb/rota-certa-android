...
object FarolSemanticCardStage32 {
    ...
    class SemanticGate {
        ...
        private var candidateSignature: String? = null
        private val slotText = LinkedHashMap<String, String>()
        private var fingerprint = stableHash64("empty")
        private var lastSnapshot = Snapshot(0L, fingerprint, null, Int.MIN_VALUE, "", "")

        @Synchronized
        fun observe(signal: Signal): Decision {
            val normalizedOwner = firstNonBlank(
                normalizePackage(signal.sourcePackage),
                normalizePackage(signal.triggerPackage),
                normalizePackage(signal.windowPackage),
            )
            val slot = canonical(signal.sourceSlot).ifBlank { "window:${signal.windowId}" }
            val text = canonicalSemanticSource(signal.sourceText).take(1800)
            val ownerChanged = owner != null && normalizedOwner != null && owner != normalizedOwner
            val windowChanged = windowId != Int.MIN_VALUE && signal.windowId >= 0 && windowId != signal.windowId
            var mutation = false
            var strong = false
            var reason = "unproven_noise"

            if (owner == null && normalizedOwner != null) owner = normalizedOwner
            if (windowId == Int.MIN_VALUE && signal.windowId >= 0) windowId = signal.windowId

            if (ownerChanged || windowChanged) {
                owner = normalizedOwner ?: owner
                if (signal.windowId >= 0) windowId = signal.windowId
                slotText.clear()
                candidateSignature = null
                if (text.isNotBlank()) slotText[slot] = text
                mutation = true
                strong = true
                reason = if (ownerChanged) "owner_transition" else "window_transition"
            } else if (text.isNotBlank()) {
                val previous = slotText[slot]
                if (previous == null) {
                    slotText[slot] = text
                    mutation = true
                    strong = true
                    reason = "new_semantic_source"
                } else if (previous != text) {
                    slotText[slot] = text
                    mutation = true
                    strong = true
                    reason = "semantic_source_changed"
                } else {
                    reason = "same_semantic_source"
                }
            } else {
                val previous = slotText[slot]
                if (previous != null) {
                    slotText.remove(slot)
                    mutation = true
                    strong = true
                    reason = "semantic_source_cleared"
                } else {
                    // Blank content-change/noise is explicitly not semantic authority.
                    reason = "blank_event_not_semantic_authority"
                }
            }
            ...
            if (mutation) {
                generation += 1L
                candidateSignature = null
                Metrics.increment("semanticMutations")
            } else {
                Metrics.increment("rawEventsPreserved")
            }
            fingerprint = computeFingerprint(owner, windowId, candidateSignature, slotText)
            lastSnapshot = Snapshot(generation, fingerprint, owner, windowId, slot, text)
            return Decision(mutation, strong, generation, fingerprint, reason, lastSnapshot)
        }

        @Synchronized
        fun observeCandidate(addressSignature: String): Decision {
            val signature = canonical(addressSignature)
            if (signature.isBlank()) {
                val snap = snapshotLocked()
                return Decision(false, false, generation, fingerprint, "blank_candidate_signature", snap)
            }
            val previous = candidateSignature
            val changed = previous != null && previous != signature
            candidateSignature = signature
            if (changed) {
                generation += 1L
                Metrics.increment("candidateSemanticMutations")
            }
            fingerprint = computeFingerprint(owner, windowId, candidateSignature, slotText)
            lastSnapshot = Snapshot(generation, fingerprint, owner, windowId, "candidate", signature)
            val reason = when {
                previous == null -> "candidate_signature_bound"
                changed -> "candidate_signature_changed"
                else -> "same_candidate_signature"
            }
            return Decision(changed, changed, generation, fingerprint, reason, lastSnapshot)
        }

        @Synchronized
        fun markReadingOff(): Snapshot {
            generation += 1L
            owner = null
            windowId = Int.MIN_VALUE
            candidateSignature = null
            slotText.clear()
            fingerprint = stableHash64("off:$generation")
            lastSnapshot = Snapshot(generation, fingerprint, null, Int.MIN_VALUE, "", "")
            Metrics.increment("semanticResetReadingOff")
            return lastSnapshot
        }

        @Synchronized fun snapshot(): Snapshot = snapshotLocked()
        @Synchronized fun lease(): Lease = Lease(generation, fingerprint)
        @Synchronized fun isFresh(lease: Lease): Boolean = lease.generation == generation && lease.fingerprint == fingerprint

        private fun snapshotLocked(): Snapshot = lastSnapshot.copy(generation = generation, fingerprint = fingerprint)
    }
