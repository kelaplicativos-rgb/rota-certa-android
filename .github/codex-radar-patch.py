...
fun openAppLocationSettings(context: Context) {
    ...
    runCatching { context.startActivity(intent) }
        .onFailure { Toast.makeText(context, "Nao consegui abrir as permissoes do app.", Toast.LENGTH_SHORT).show() }
}
...
fun AlwaysLocationPermissionCard() {
    ...
    val allowed = hasAlwaysLocationPermission(context)
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Text("GPS continuo", fontWeight = FontWeight.Bold)
            Text(
                if (allowed) {
                    "Localizacao configurada para uso continuo. Alertas de proximidade e radares podem funcionar em segundo plano."
                } else {
                    "Para radares e alertas funcionarem com o app em segundo plano, abra Localizacao e selecione Permitir o tempo todo."
                },
                style = MaterialTheme.typography.bodySmall,
            )
            OutlinedButton(onClick = { openAppLocationSettings(context) }, modifier = Modifier.fillMaxWidth()) {
                Text(if (allowed) "Abrir permissoes de localizacao" else "Ativar Permitir o tempo todo")
            }
        }
    }
}
...
def patch_manifest():
    ...
    text = path.read_text()
    if "ACCESS_BACKGROUND_LOCATION" not in text:
        text = text.replace(
...
def patch_repository():
    ...
    text = path.read_text()
    if "importedRadarsKey" not in text:
        text = replace_once(text,
