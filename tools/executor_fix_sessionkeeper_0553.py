...
replacements = {
    ...
    "actual == null && current?.state.isStickyFailure() -> current.copy(": "actual == null && current != null && current.state.isStickyFailure() -> current.copy(",
...
# Fail closed if any nullable sticky guard that prevents Kotlin smart-casting remains.
if "current?.state.isStickyFailure()" in text:
    raise SystemExit("nullable sticky guard remains; refusing to continue")

required = [
    "current != null && current.state.isStickyFailure()",
    "actual == null && current != null && current.state.isStickyFailure() -> current.copy(",
]
