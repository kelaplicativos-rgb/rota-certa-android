...
class LiveRideRouteCachePersistenceTest {
    ...
    fun changingRadiusCreatesDifferentOperationalCacheKey() {
        ...
        assertTrue(
            LiveRideRouteCache.keyFor(fields, radiusFive) !=
                LiveRideRouteCache.keyFor(fields, radiusTen),
        )
    }

    @Test
    fun malformedPayloadIsIgnoredWithoutCrashing() {
        val cache = LiveRideRouteCache()
        assertEquals(0, cache.importSnapshot("invalid\nnot-a-route"))
