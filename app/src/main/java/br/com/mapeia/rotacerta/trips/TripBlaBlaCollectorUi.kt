...
fun BlaBlaCollectorPanel(
    ...
    val manualSeatLedger = remember(context) { BlaBlaManualSeatSyncLedger(context) }
    var revision by remember { mutableIntStateOf(0) }
    var syncing by remember { mutableStateOf(false) }
    var archiving by remember { mutableStateOf(false) }
    var manualSeatSyncing by remember { mutableStateOf(false) }
    var activeManualSeatRequest by remember { mutableStateOf<BlaBlaManualSeatSyncRequest?>(null) }
    ...
    fun exposeNextQueuedManualRequest() {
        if (manualSeatStore.peek() != null && autoSyncToken > 0) {
            handledAutoSyncToken = (autoSyncToken - 1).coerceAtLeast(0)
        }
    }
    ...
    val seatSyncLauncher = rememberLauncherForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        ...
        val completedRequest = activeManualSeatRequest
        activeManualSeatRequest = null
        val accountId = result.data?.getStringExtra(BlaBlaManualSeatAutomationIntents.EXTRA_ACCOUNT_ID)
        ...
        if (result.resultCode == Activity.RESULT_OK && completedRequest != null) {
            if (completedRequest.seatDelta < 0) {
                manualSeatLedger.markVerifiedDecrease(completedRequest)
            } else if (completedRequest.seatDelta > 0) {
                manualSeatLedger.clearAfterVerifiedReverse(completedRequest.localBookingId)
            }
            UnifiedDebugEventStore.record(
                "EXTERNAL_SEAT_SYNC_LEDGER_UPDATED",
                context.packageName,
                "request=${completedRequest.id} delta=${completedRequest.seatDelta} verified=true",
            )
        }
        message = seatMessage
    ...
    LaunchedEffect(autoSyncToken, autoSyncProfileUuid, syncing, archiving, manualSeatSyncing, handledAutoSyncToken, accounts.size) {
        ...
        if (pendingManualSeat != null) {
            ...
            activeManualSeatRequest = pendingManualSeat
