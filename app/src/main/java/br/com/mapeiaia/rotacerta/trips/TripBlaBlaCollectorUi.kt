...
fun BlaBlaCollectorPanel(
    ...
    val scriptWorkspace0486 = remember(context) { BlaBlaScriptWorkspace0486(context) }
        ...
        AlertDialog(
            ...
            text = {
                Column(
                    ...
                    Text("Scripts", style = androidx.compose.material3.MaterialTheme.typography.titleSmall)
                    Text(
                        "${dateScopeSelectedScripts0449.size}/${BlaBlaDateScopeScriptCatalog0449.all.size} ativos para esta sincronização. " +
                            "A seleção e o código de cada script agora ficam centralizados no menu Scripts.",
                    )
                    Text(
                        "Edite, cole, corrija, ative/desative ou restaure scripts em ☰ → Scripts. " +
                            "Esta tela usa exatamente a seleção salva nessa central.",
                        style = androidx.compose.material3.MaterialTheme.typography.bodySmall,
                    )
                    if (dateScopeSelectedScripts0449.isEmpty()) {
                        Text("⚠️ Nenhum script está ativo. Abra ☰ → Scripts antes de iniciar.")
                    }
