...
internal fun planTimelineExternalCapacityClaims(
    ...
    val activeLocalExternal = existingBookings.filter { booking ->
        booking.tripId == trip.id &&
            booking.source == BookingSource.BLABLACAR &&
            booking.capacityClaimType == CapacityClaimType.PASSENGER &&
            booking.seats > 0 &&
            (booking.status == BookingStatus.CONFIRMED ||
                (booking.status == BookingStatus.HELD && (booking.holdExpiresAtMillis == null || booking.holdExpiresAtMillis > now)))
    }
    val localStableRefs = activeLocalExternal.mapNotNull { it.sourceReference.trim().takeIf(String::isNotEmpty) }.toSet()
    val representedLocally = activeLocalExternal.sumOf(Booking::seats)
    ...
    val mirrorTarget = (externalSeatEvidence - representedLocally).coerceAtLeast(0)
    if (mirrorTarget == 0) return emptyList()

    val stableClaims = mutableListOf<Booking>()
    var stableSeats = 0
    entry.blablaPassengers.forEach { passenger ->
        val href = passenger.booking_href?.trim()?.takeIf(String::isNotEmpty) ?: return@forEach
        if (href in localStableRefs) return@forEach
        ...
        val from = if (boardingIndex >= 0 && dropoffIndex > boardingIndex) stops[boardingIndex] else stops.first()
        val to = if (boardingIndex >= 0 && dropoffIndex > boardingIndex) stops[dropoffIndex] else stops.last()
        val refHash = sha256Short0256("$externalKey|$href", 24)
        stableClaims += Booking(
            id = "timeline-ext-seat-$refHash",
            tripId = trip.id,
            passengerName = "Reserva externa",
    ...
    val result = stableClaims.toMutableList()
    val gap = (mirrorTarget - stableSeats).coerceAtLeast(0)
    if (gap > 0) {
        val aggregateHash = sha256Short0256("$externalKey|aggregate", 24)
        result += Booking(
            id = "timeline-ext-seat-$aggregateHash",
            tripId = trip.id,
            passengerName = "Reservas externas não segmentadas",
            ...
            source = BookingSource.BLABLACAR,
            capacityClaimType = CapacityClaimType.RESERVED_SEAT,
            sourceReference = "$EXTERNAL_CAPACITY_PREFIX$aggregateHash",
            createdAtMillis = now,
            updatedAtMillis = now,
        )
    }
