...
internal fun planTimelineExternalCapacityClaims(
    ...
    // Any already-persisted BlaBlaCar capacity claim except the synthetic claims
    // generated by this reconciler is treated as real local evidence. We never
    // join it to a passenger by name/phone/reference; only the physical segment
    // load is subtracted from the external load. This prevents legacy records
    // without sourceReference from being counted twice.
        ...
        .forEach { booking ->
            ...
            if (fromIndex >= 0 && toIndex > fromIndex) {
                for (segment in fromIndex until toIndex) {
                    localExternalBySegment[segment] += booking.seats
                }
            }
        }

    // External passenger rows with a provable route contribute only to the
    // segments they actually occupy. Rows whose route is not provable remain
    // unsegmented and are conservatively applied to every segment.
    ...
    entry.blablaPassengers.forEach { passenger ->
        ...
        val boardingIndex = timelineStopIndexForLabel0256(
            stops,
            passengerTimelinePlaceLabel(passenger.name, passenger.boarding),
        )
        val dropoffIndex = timelineStopIndexForLabel0256(
            stops,
            passengerTimelinePlaceLabel(passenger.name, passenger.dropoff),
        )
        if (boardingIndex >= 0 && dropoffIndex > boardingIndex) {
            mappedPassengerSeats += seats
            for (segment in boardingIndex until dropoffIndex) {
                mappedExternalBySegment[segment] += seats
            }
        }
    }
