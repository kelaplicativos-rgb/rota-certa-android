...
internal fun canonicalTimelineProjection0494(
    ...
    winners.forEach { state ->
        ...
        projectedEntries += TripTimelineEntry(
            ...
            departureAtMillis = state.departureAtMillis,
            arrivalAtMillis = canonicalAgendaArrivalAtMillis0581(
                departureAtMillis = state.departureAtMillis,
                plannedArrivalMillis = state.arrivalAtMillis.takeIf { it > 0L } ?: destination.plannedArrivalMillis,
                plannedDepartureMillis = destination.plannedDepartureMillis,
            ),
            origin = origin.name,
