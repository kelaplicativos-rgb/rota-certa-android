...
class DirectionalProximityAlertEngine(
    ...
    private fun radarTitle(radar: ImportedRadar): String = when (val speed = radar.speedKmh) {
        null -> "Radar à frente"
        else -> "Radar de $speed km/h"
    }
    ...
    private data class Candidate(
        ...
        fun toVisual(fix: PreciseNavigationFix): DirectionalAlertVisual = DirectionalAlertVisual(
            ...
            speedKilometersPerHour = fix.speedKilometersPerHour,
            headingSource = fix.headingSource,
            status = "Aproximando — sentido confirmado",
            gpsReliable = true,
            shouldClose = false,
            savedPlaceId = alert?.id,
            radarId = radar?.id,
    ...
    private companion object {
        ...
        const val REQUIRED_APPROACHING_SAMPLES = 1
