...
//
// A interface le os arquivos somente quando o usuario abre o modulo de cards.
// A bolinha continua salvando em Dispatchers.IO e possui uma limpeza periodica
// independente, portanto galeria, preview e atalhos nao entram no caminho critico.

fun findFunctionEnd128(source: String, start: Int): Int {
...
fun patchAutomaticCaptureGallery128(
    ...
    if ("automatic_capture_gallery_ui_0_1_128" !in main) {
        ...
        val cardEnd = findFunctionEnd128(main, cardStart)
...
"""
    ...
    if ("automatic_capture_periodic_cleanup_0_1_128" !in service) {
        val fieldAnchor = "    private var lastAutomaticCaptureAtMillis128: Long = 0L\n"
        if (fieldAnchor !in service) throw GradleException("Campos da captura automatica nao encontrados no servico.")
...
tasks.matching { it.name == "preBuild" }.configureEach {
    doLast {
        patchAutomaticCaptureGallery128(
