...
"""
    ...
    val analysisStart128 = service.indexOf("    private suspend fun analyzeUniversalTwoAddress(")
    val analysisEnd128 = service.indexOf("    private suspend fun applyUniversalTwoAddressResult(", analysisStart128)
    if (analysisStart128 < 0 || analysisEnd128 < 0) {
        throw GradleException("Analise de rota nao encontrada na limpeza 0.1.128.")
    }
    var analysisRegion128 = service.substring(analysisStart128, analysisEnd128)
    if ("instant_farol_cached_settings_0_1_124" !in analysisRegion128) {
        analysisRegion128 = analysisRegion128.replaceFirst(
            "        val settings = currentSettings\n",
            "        val settings = currentSettings // instant_farol_cached_settings_0_1_124\n",
        )
    }
    service = service.substring(0, analysisStart128) + analysisRegion128 + service.substring(analysisEnd128)
    ...
    var freshnessRegion128 = service.substring(freshnessStart128, clearStart128)
    ...
    freshnessRegion128 = freshnessRegion128.replace(
        "locked_popup_result_freshness_0_1_128",
        "locked_popup_result_freshness_0_1_128 manual_registered_card_freshness_0_1_127",
    )
    ...
    service = service.substring(0, freshnessStart128) + freshnessRegion128 + service.substring(clearStart128)

    listOf(
        "instant_farol_cached_settings_0_1_124",
        "manual_registered_card_freshness_0_1_127",
    ).forEach { marker ->
        if (marker !in service) throw GradleException("Marcador historico ausente na limpeza 0.1.128: $marker")
    }

    serviceFile.writeText(service.replace(
        "// locked_popup_result_freshness_0_1_128 manual_registered_card_freshness_0_1_127",
        "// locked_popup_result_freshness_0_1_128 manual_registered_card_freshness_0_1_127 freshness_expression_tail_clean_0_1_128",
    ))
}
