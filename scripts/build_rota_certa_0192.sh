...
# 1) Reproduz exatamente a 0.1.191 já aprovada em CI.
bash "$PATCH_REPOSITORY/scripts/build_rota_certa_0191.sh" "$PATCH_REPOSITORY"
...
# 2) A 0.1.192 altera somente versão, controlador do overlay, integração do serviço e teste.
PROTECTED_FILES=(
...
python3 "$PATCH_REPOSITORY/scripts/apply_alert_popup_hold_0192.py" "$SOURCE_REPOSITORY"
...
grep -Fq 'if (pendingClose != null) return' app/src/main/java/br/com/mapeiaia/rotacerta/DirectionalAlertOverlayController.kt
grep -Fq 'directionalAlertOverlayChecklist5.hideFromEngineIdle()' app/src/main/java/br/com/mapeiaia/rotacerta/LiveRideAccessibilityService.kt
! grep -Fq $'if (visual == null) {\n                    directionalAlertOverlayChecklist5.hide()' app/src/main/java/br/com/mapeiaia/rotacerta/LiveRideAccessibilityService.kt
...
test -f app/src/test/java/br/com/mapeiaia/rotacerta/AlertPopupPostPassHold0192ContractTest.kt

echo 'alert_popup_post_pass_hold_0192_contracts=passed'
...
if count <= 0:
    raise SystemExit('Nenhum teste foi descoberto na 0.1.192')
if failures:
    raise SystemExit('Há testes com falha na 0.1.192')
PY
...
OUTPUT_DIR="artifact-0.1.192"
APK_NAME="rota-certa-0.1.192-popup-radar-alerta-3s-validado-em-ci.apk"
test -s "$APK_SOURCE"
rm -rf "$OUTPUT_DIR"
mkdir -p "$OUTPUT_DIR"
...
grep -F "package: name='br.com.mapeiaia.rotacerta' versionCode='5476' versionName='0.1.192'" "$OUTPUT_DIR/badging.txt"
printf '%s\n' 'br.com.mapeiaia.rotacerta' > "$OUTPUT_DIR/package.txt"
printf '%s\n' 'versionName=0.1.192' 'versionCode=5476' > "$OUTPUT_DIR/version.txt"
...
engine_idle_preserves_pending_post_pass_close=true
explicit_hide_remains_immediate=true
...
echo 'rota_certa_0192_build=passed'
