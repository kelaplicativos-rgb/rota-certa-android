...
fun RotaCertaApp(launchIntent: Intent?) {
    ...
    LaunchedEffect(launchIntent) {
        ...
        moduleNavigationActive0172 = highlightedShortcutModule0171 != null || tab != TAB_CONFIG || selectedBubbleGroup != BUBBLE_GROUP_GENERAL
    ...
    BackHandler(
        enabled = moduleNavigationActive0172 || tab != TAB_CONFIG || selectedBubbleGroup != BUBBLE_GROUP_GENERAL || highlightedShortcutModule0171 != null,
    ) {
        tab = TAB_CONFIG
...
private fun ShortcutModulesHome0171(
    ...
    val context = LocalContext.current
    LaunchedEffect(Unit) {
        check(HomeModuleExpansionPolicy0174.CONTRACT_MARKER.isNotBlank())
    ...
    Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        Text(
            "Toque em um módulo para mostrar seus controles imediatamente dentro do próprio cartão. Apenas um módulo permanece aberto por vez.",
            style = MaterialTheme.typography.bodySmall,
        )
        Text("Ações fixas na grade", fontWeight = FontWeight.Bold)
        Text(
            "A grade flutuante continua com ações fixas. A Home serve para configurar e usar cada recurso sem procurar o conteúdo no final da página.",
            style = MaterialTheme.typography.bodySmall,
        )
        BubbleShortcutCatalog.modules.forEach { module ->
            ShortcutModuleCard0174(
                spec = module.spec,
                expanded = HomeModuleExpansionPolicy0174.isExpanded(expandedModuleId, module.spec.id),
                onToggle = { onToggleModule(module.spec) },
                content = { moduleContent(module.spec) },
            )
        }
    }
} // home_module_content_inline_0_1_174

@Composable
private fun ShortcutModuleCard0174(
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            if (expanded) {
                ...
                content()
                Text(
                    "Na grade: ${ShortcutGridPolicy0173.fixedBehaviorLabel(spec)}.",
                    style = MaterialTheme.typography.bodySmall,
                )
            }
        }
