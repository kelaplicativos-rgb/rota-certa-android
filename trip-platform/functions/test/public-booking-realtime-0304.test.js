...
test("0.1.626 public portal keeps known devices authenticated and routes to Minha área", () => {
  assert.match(html, /id="passengerAreaLink0589"[^>]*>Minha área<\/a>/);
  assert.match(web, /probePassengerSession0626/);
  assert.match(web, /credentials: "same-origin"/);
  assert.doesNotMatch(web, /sessionStorage\.setItem\(passengerSessionKey0625, passengerSessionToken0623\)/);
